...
for dataset in datasets:
    ...
    # df = pd.read_csv(dataset, nrows=50)
    ...
    for i in range(len(ACTIVITIES)):    # num classes
        ...
        for j in range(X_train.shape[1]):   # num features
            ...
            _mean_feature = np.nan_to_num(np.mean(X_train_N_arr[i][:,j].tolist()))
            ...
            _var_feature = np.nan_to_num(np.var(X_train_N_arr[i][:,j].tolist()))
    ...
    for i in range(X_test.shape[0]):    # each row
        ...
        for j in range(len(ACTIVITIES)):    # each class
            ...
            psb.cal_parameters(f, _vars_N[j], _means_N[j])
        # print(psb.BX)
            
            # tx_receipt = cal_class(f, _vars_N[j], _means_N[j], account1, private_key1, cont_pred)
            # gas_per_row += tx_receipt.gasUsed
        
        psb.cal_probability(priorN[0],priorN[1],priorN[2],priorN[3],priorN[4],priorN[5])
