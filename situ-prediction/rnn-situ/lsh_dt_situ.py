...
def add_noise(input_list): # len(input_list) = 435
    
    # Reshape the list to a 2D array (15x29)
    reshaped_array = np.array(input_list).reshape(15, 29)

    # Select n rows following a normal distribution
    n = int(np.random.normal(7.5, 2.5))
    n = max(1, min(15, n))

    # Randomly flip 30% of the binaries in the selected n rows
    for _ in range(n):
        row_idx = random.randint(0, 14)  # Select a random row index
        row = reshaped_array[row_idx]
        num_flips = int(0.2 * len(row))  # Calculate the number of flips (30% of the row length)
        flip_indices = random.sample(range(len(row)), num_flips)  # Randomly select indices to flip
        for idx in flip_indices:
            row[idx] = 1 - row[idx]  # Flip the binary value

    return reshaped_array.flatten().tolist()
...
for dataset in datasets:
    ...
    for i in range(0, len(data) - WINDOW_SIZE + 1):
        ...
        X_window = data.drop(columns=['Activity', 'timestamp']).iloc[i:i+WINDOW_SIZE].values.flatten().tolist()
        y_label = data['Activity'].iloc[i + WINDOW_SIZE - 1]
...
####################### LSH
    ...
    predictions = [lsh.predict(add_noise(X)) for X in X_test]
